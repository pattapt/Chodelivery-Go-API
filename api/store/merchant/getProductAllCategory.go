...
func GetProductsByCategoryId(c *gin.Context) {
	...
	categoryToken := c.Param("categoryToken")
	...
	query := `
		...
		FROM Product
		WHERE CategoryId = (SELECT CategoryId FROM Category WHERE CategoryToken = ?)
		AND MerchantId = ? AND Invisible = 'visible'
		ORDER BY CreateDate DESC
		LIMIT ?, ?
	`

	rows, err := db.Query(query, categoryToken, MerchantId, Start, limit)
	if err != nil {
		errorResponse := APIStructureMain.GeneralErrorMSG()
		c.JSON(http.StatusBadRequest, errorResponse)
		return
